                ...
                switch (item) {
                    case 0:
                        ...
                        if (cameraIntent.resolveActivity(mActivity.getPackageManager()) != null) {
                            File directoryDcim = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM) + "/"
                                            + System.currentTimeMillis() + ".jpg");
                            realPathFromUri = directoryDcim.getAbsolutePath();
                            cameraIntent.putExtra(
                                    MediaStore.EXTRA_OUTPUT, Uri.fromFile(directoryDcim));
                            mImageHandlingDelegate.handleIntent(cameraIntent, AppConstants.RequestCodes.TAKE_PHOTO);
                        }
                        break;
                    case 1:
                        Intent galleryIntent = new Intent(Intent.ACTION_PICK, MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
                        galleryIntent.setType(TYPE_IMAGE_STR);
                        mImageHandlingDelegate.handleIntent(galleryIntent, AppConstants.RequestCodes.PICK_FROM_GALLERY);
                        break;
                    case 2:
                      /*  Intent fullScreenIntent = new Intent(mActivity, FullScreenImageViewActivity.class);
                        mImageHandlingDelegate.handleIntent(fullScreenIntent, AppConstants.RequestCodes.SEND_IMAGE_PATH);
                 */   default:
                        break;
                }
            }
